  ...
  {(()=>{
    ...
    return(<>
      ...
      <div className="card" style={{marginTop:16}}>
        ...
        {/* ── REGISTROS ── */}
        {tab==="registros"&&<>
          <div className="pt">Todos os Lançamentos</div>
          <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
            <select className="fi" value={fType} onChange={e=>setFType(e.target.value)} style={{flex:"1 1 120px"}}>
              <option value="todos">Todos os tipos</option>
              <option value="receita">↑ Receitas</option>
              <option value="despesa">↓ Despesas</option>
            </select>
            <select className="fi" value={fCard} onChange={e=>setFCard(e.target.value)} style={{flex:"1 1 140px"}}>
              <option value="todos">Todos os cartões</option>
              {Object.values(CARDS).filter((c,i,a)=>a.findIndex(x=>x.name===c.name)===i).map(c=><option key={c.name} value={c.name}>{c.name}</option>)}
              {Object.values(CARDS).filter((c,i,a)=>a.findIndex(x=>x.owner===c.owner)===i).map(c=><option key={c.owner} value={c.owner}>{c.owner}</option>)}
            </select>
            <select className="fi" value={fCat} onChange={e=>setFCat(e.target.value)} style={{flex:"1 1 140px"}}>
              <option value="todos">Todas categorias</option>
              {[...CATS.despesa,...CATS.receita].map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
            </select>
          </div>
          {filtered.length===0?<div className="empty"><div style={{fontSize:32,marginBottom:8}}>🔍</div>Nenhum registro encontrado</div>:filtered.map(r=><RecItem key={r.id} r={r}/>)}
        </>}
